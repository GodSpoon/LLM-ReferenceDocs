...
This script is based on the [original article](https://petri.com/EXAMPLE_SECRET_VALUE_PLACEHOLDER) written by [Tony Redmond](https://twitter.com/12Knocksinna).
...
<Tabs>
  ...
  function Test-GroupMembership {
    ...
    if ($Group.Membership.Owners.Count -le 1 -and ($Group.Membership.Members.Count + $Group.Membership.Guests.Count) -eq 0) {
      Write-Host "  → potentially obsolete (abandoned group: only $($Group.Membership.Owners.Count) owner left)" -ForegroundColor Yellow
  ...
  function Test-ConversationActivity {
    ...
    $EXAMPLE_SECRET_VALUE_PLACEHOLDER = $outdatedConversations | Sort-Object -Property lastDeliveredDateTime
  ...
  function New-Report {
    ...
    $exportObject = [PSCustomObject][ordered] @{
      ...
      "Number of Conversations" = $EXAMPLE_SECRET_VALUE_PLACEHOLDER ? $EXAMPLE_SECRET_VALUE_PLACEHOLDER : "n/a"
      "Last Conversation"       = $Group.MailboxStatus.LastConversation
      "Conversation Status"     = $Group.MailboxStatus.Reason ?? "Normal"
      "Team enabled"            = $EXAMPLE_SECRET_VALUE_PLACEHOLDER -contains 'Team' ? "True" : "False"
